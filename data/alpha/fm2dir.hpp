...
public:
    ...
    virtual void setInitialAndGoalPoints(const std::vector<int> &initial_point, const std::vector<int> &fmm2_sources, const int goal_idx)
    {
     ...
     * @see init()
     */
    virtual void setInitialPoints(const std::vector<int> &init_points, const bool save_velocity = false)
    {
    ...
    */
    virtual void init(const bool save_velocity = false, const bool directional = false)
    {
     ...
     */
    virtual double solveEikonal(const int &idx, const int &idx_source = -1)
    {
    ...
    virtual void computeFM(const bool stop = true, const bool directional = false)
    {
     ...
     * @see setInitialPoints()
     */
    virtual void computeFM2Directional(const float maxDistance = -1)
    {
     ...
     */
    virtual void computePath(path_t *p, std::vector<double> *path_velocity)
    {
        path_t *path_ = p;
     ...
     * */
    double getMinValueInDimDirectional(const int idx, const int dim)
    {
...
private:
    ...
    void computeVelocitiesMap(bool saturate = false)
    {
