...
public:
    ...
    bool readOptions(const char *filename)
    {
        ...
        desc.add_options()("grid.file", boost::program_options::value<std::string>(), "Path to load a velocities map from image.")("grid.text", boost::program_options::value<std::string>(), "Path to load a velocities map from a .grid file.")("grid.ndims", boost::program_options::value<std::string>()->default_value("2"), "Number of dimensions.")("grid.cell", boost::program_options::value<std::string>()->default_value("FMCell"), "Type of cell. FMCell by default.")("grid.dimsize", boost::program_options::value<std::string>()->default_value("200,200"), "Size of dimensions: N,M,O...")("grid.leafsize", boost::program_options::value<std::string>()->default_value("1"), "Leafsize (assuming cubic cells).")("problem.start", boost::program_options::value<std::string>()->required(), "Start point: s1,s2,s3...")("problem.goal", boost::program_options::value<std::string>()->default_value("nan"), "Goal point: g1,g2,g3... By default no goal point.")("benchmark.name", boost::program_options::value<std::string>()->default_value(name.string()), "Name of the benchmark.")("benchmark.runs", boost::program_options::value<std::string>()->default_value("10"), "Number of runs per solver.")("benchmark.savegrid", boost::program_options::value<std::string>()->default_value("0"), "Save grid values of each run.");
    ...
    void configure(Benchmark<grid_t> &b)
    {
        ...
        for (size_t i = 0; i < solverNames_.size(); ++i)
        {
            ...
            { // Create solvers with specified constructor parameters.
