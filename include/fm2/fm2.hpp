...
public:
    ...
    FM2(double maxDistance = -1) : Solver<grid_t>("FM2"), maxDistance_(maxDistance)
    {
        solver_ = new FMM<grid_t, heap_t>();
    }

    /** \brief maxDistance sets the velocities map saturation distance in real units (before normalization). */
    FM2(const char *name, double maxDistance = -1) : Solver<grid_t>(name), maxDistance_(maxDistance)
    {
        solver_ = new FMM<grid_t, heap_t>();
    }
