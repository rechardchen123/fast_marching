...
void FastMarching<DIM, Pixel, sign>::init_trial_from_alive()
{
    ...
    for (unsigned int j = 0; j < nb_alive_points(); j++)
    {
        ...
        for (int i = 1; i <= DIM; i++)
        {
            ...
            if (sup(i) < m_phi->extent(i - 1))
                __update_point(sup); // add_trial_point(sup);
            if (inf(i) >= 0)
                __update_point(inf); // add_trial_point(inf);
        }
    }
}
...
Pixel Eikonal<DIM, Pixel, sign>::_update_value(const Index &ind) const
{
    ...
    for (int i = 1; i <= DIM; i++)
    {
        ...
        if (sup(i) >= base::extent(i - 1))
            val[i - 1] = base::_get_value(inf);
        else if (inf(i) < 0)
            val[i - 1] = base::_get_value(sup);
        else
            val[i - 1] = std::min(base::_get_value(inf), base::_get_value(sup));
    }
