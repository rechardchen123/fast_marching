...
class nDGridMap
{
    friend std::ostream &operator<<(std::ostream &os, nDGridMap<T, ndims> &g)
    {
        ...
        for (unsigned int i = 0; i < ndims; ++i)
            os << "\t\t"
               << "d" << i << "\tsize: " << g.dimsize_[i] << std::endl;
...
public:
    ...
    nDGridMap(const std::array<unsigned int, ndims> &dimsize, double leafsize = 1.0f) : leafsize_(leafsize),
                                                                                        clean_(true)
    {
        resize(dimsize);
    }
    ...
    inline void setOccupiedCells(const std::vector<unsigned int> &obs)
    {
        occupied_ = obs;
    }

    /** \brief Sets (by move semantics) the cells which are occupied. Usually called by grid loaders. */
    inline void setOccupiedCells(std::vector<unsigned int> &&obs)
    {
        occupied_ = std::move(obs);
    }

    /** \brief Returns the indices of the occupied cells of the grid. */
    inline void getOccupiedCells(std::vector<unsigned int> &obs) const
    {
        obs = occupied_;
    }
